# Tasarım Sistemi Planı — EPİAŞ Hesaplayıcı
...
### Adım 0 — Pre-flight (bağımsız, sıfır stil etkisi)

- `<html data-bs-theme="dark">` → `<html lang="tr">` (dark attribute silinir)
- `design-system.css` dosyası oluşturulur, `<link>` olarak Bootstrap'ten hemen sonra eklenir. Başlangıçta sadece token'lar içerir — henüz override yok.

> Bu iki değişiklik sonrasında sayfa "bozuk" görünür (dark arka plan gider, Bootstrap default light tema devreye girer). Bu beklenen geçiş anı — bir sonraki adım hemen onu düzeltir.

---

### Adım 1 — Foundation: `design-system.css`

Tek dosyada sırayla:

1. `:root` → tüm token'lar
2. `body` → font stack, bg, color, line-height reset
3. Bootstrap global override'ları (renk, buton, card, alert, form, tablo, nav, dropdown, spinner)
4. Utility class'ları (`.num`, `.dim`, `.btn-sm`, `.btn-lg` vb.)
5. Component class'ları (`.radio-group`, `.result-card`, `.stat-card`, `.validation-panel`, `.empty-state` vb.)

`<head>` sırasında: Bootstrap CSS → `design-system.css` → (ia-ptf.css geçici olarak kalır, Adım 4'te kaldırılır).

---

### Adım 2 — `index.html` `<style>` Bloğunu Boşalt

Satır 10–443 arasındaki `<style>` bloğu tamamen silinir. İçerdiği her şey ya:
- Gereksizdir (dark theme renkleri, gradient'lar) — çöpe
- `design-system.css`'e taşınmıştır (typography, form, card, stat-card, table stilleri)

CSS değişkenleri (`--dark-bg`, `--primary-gradient`, vb.) `design-system.css`'deki token'larla replace edilir.

---

### Adım 3 — Sekme 1: Ödeme Takvimi

Değişiklikler:

| Element | Mevcut | Yeni |
|---------|--------|------|
| Nav wrapper | `style="background: var(--dark-card)..."` inline | Sadece `.tabs` class — token override halleder |
| Tab button'lar | `style="border-radius: 10px; font-weight: 600;"` inline | `.tab` class — token halleder |
| Hızlı Hesaplama card-header | `style="background: linear-gradient(45deg, #6f42c1, #007bff)..."` | Silinir, `.card-header` yeterli |
| Dönem Analizi card-header | `style="background: linear-gradient(45deg, #fd7e14, #dc3545)..."` | Silinir |
| "Takvim Oluştur" butonu | `btn-danger` | `btn-primary` |
| "Hesapla" butonu | `btn-primary` | `btn-primary` (override otomatik etki eder) |
| "İndir" dropdown | `btn-success` | `btn-secondary` |
| `#quickResult result-card` | gradient bg | `.result-card` (accent-subtle bg + sol border) |
| Tablo | `table-dark table-hover` + `style="background: linear-gradient(45deg, #20c997, #28a745)..."` thead | `table` class, `table-dark` kaldırılır, inline style kaldırılır |
| Stat card'lar | `background: var(--primary-gradient); -webkit-background-clip: text` gradient text | `--accent` düz renk, gradient text yok |
| Loading spinner | `text-primary` class | Silinir, `--accent` override otomatik |

---

### Adım 4 — Sekme 2: Faiz Hesaplama

| Element | Mevcut | Yeni |
|---------|--------|------|
| Bilgilendirme alert | `style="background: linear-gradient(45deg, #17a2b8, #20c997)..."` | `.alert-info` (override halleder, inline style silinir) |
| Dönem Seçimi card-header | gradient inline style | Silinir |
| Faiz Formu card-header | gradient inline style | Silinir |
| Faiz türü radio | `btn-check + btn-outline-success` | `.radio-group` markup'ına dönüştürülür |
| Dönüşüm result alert | gradient inline style | `.alert-info` |
| Sonuç card | `style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%)..."` büyük mor kart | `.card` + `.bg-subtle` — gradient komple kaldırılır |
| İkon'lar (3 adet, 2.5rem) | `style="font-size: 2.5rem; color: rgba(255,255,255,0.9)..."` | `--text-xl`, `--accent` renk, inline style silinir |
| `details-card` glassmorphism | `background: rgba(255,255,255,0.1); backdrop-filter: blur(10px)` | `.card` — glassmorphism kaldırılır |
| Faiz Hesapla butonu | `btn-success` | `btn-primary` |

---

### Adım 5 — Sekme 3: İA-PTF

Bu adım diğerlerinden farklı: `ia-ptf.css` kaldırılır, içeriği `design-system.css`'e absorb edilir, class isim değişiklikleri yapılır.

**ia-ptf.css'deki token'lar:**
- `#ia-ptf-pane` scope'undaki `--accent`, `--danger` vb. → `design-system.css`'deki global token'larla aynı, silinir.
- Pane-spesifik `--bg-page: #ffffff`, `--bg-card: #ffffff` vb. → global `--bg-page: #f9fafb`'ye bırakılır. Tek fark: ia-ptf paneli `--bg-page: #f9fafb` yerine `#ffffff` istiyorsa, pane-level override kalabilir — ancak global token yeterli olabilir, test edilecek.

**Class isim eşleştirmesi:**

| Eski (`ia-ptf.css`) | Yeni (`design-system.css`) | JS değişikliği |
|---------------------|---------------------------|---------------|
| `.ia-btn-primary` | `.btn-primary` | `ia-ptf.js`'de string replace |
| `.ia-btn-secondary` | `.btn-secondary` | `ia-ptf.js`'de string replace |
| `.ia-step-card` | `.card` (header/body zaten var) | HTML'de replace |
| `.ia-supplier-card` | `.supplier-card` (yeni class, `design-system.css`'de tanımlanır) | `ia-ptf.js`'de replace |
| `.ia-table-wrap` | `.table-wrap` | HTML + `ia-ptf.js` |
| `.ia-table-scroll` | `.table-scroll` | HTML + `ia-ptf.js` |
| `.ia-table` | `.table` (Bootstrap override halleder) | HTML + `ia-ptf.js` |
| `.ia-alert-danger` | `.alert.alert-danger` | `ia-ptf.js` |
| `.ia-alert-warning` | `.alert.alert-warning` | `ia-ptf.js` |
| `.ia-validation-panel` | `.validation-panel` | HTML + `ia-ptf.js` |
| `.ia-validation-title` | `.validation-panel-title` | `ia-ptf.js` |
| `.ia-validation-dismiss` | `.btn-icon` + `.dismiss` | `ia-ptf.js` |
| `.ia-section-title` | Silinir, `h4`/`h5` token'dan alır | HTML |
| `.ia-loading-text` | `.spinner-label` (yeni, küçük text altında) | `ia-ptf.js` |

`ia-ptf.css` dosyası silinir. `<head>`'den `<link href="ia-ptf.css">` kaldırılır.

---

### Adım 6 — Cleanup

- `index.html`'de kalan `style="..."` attribute taraması: hepsinin gittiği doğrulanır
- `grep -r "linear-gradient"` → sıfır sonuç olmalı
- `grep -r "#667eea\|#764ba2\|#58a6ff\|#0d1117\|#161b22\|#30363d\|#21262d"` → sıfır sonuç olmalı
- `grep -r "Segoe UI.*Tahoma"` → sıfır sonuç olmalı
- Kullanılmayan class'lar (`holiday-item`, `badge-holiday`, `badge-weekend`, `badge-business` — sadece `<style>` bloğunda vardı, HTML'de dinamik render'lanıyordu) → JS üretim kodunda (`epias-calculator.js`) review edilir, yeni badge class'larıyla replace edilir

---

## 6. Özel Sorular — Öneriler
...
|------|----------|-----------|
| **Bootstrap tab JS** | `data-bs-toggle="pill"` → `"tab"` değişimi yanlış yapılırsa sekme switching bozulur. | HTML'de her üç nav button'u beraber değiştir. `nav-pills` → `nav-tabs` (ya da custom class) ile birlikte test et. |
| **`d-none` JS bağımlılığı** | `epias-calculator.js` ve `ia-ptf.js` show/hide için `classList.remove('d-none')` / `add('d-none')` kullanıyor. Bootstrap'in `d-none` `display: none !important` tanımı korunmalı. | `design-system.css`'de `d-none`'ı override ETME. Bootstrap utility olarak kalsın. |
| **`form-control.is-invalid` JS** | `ia-ptf.js` validation sırasında `is-invalid` class'ı form-control'lara ekliyor. Override bunu desteklemeli. | Override'da `border-color: var(--danger) !important` yeterli — yapısal değişiklik yok. |
| **Bootstrap dropdown JS** | Dropdown'ın görsel override'ı JS event handler'larını etkilemez. Ama `.dropdown-toggle` class'ı kaldırılırsa Bootstrap dropdown çalışmaz. | Sadece görsel override — class isimleri HTML'de aynen kalır. |
| **`ia-ptf.js` class string'leri** | JS içinde createElement + className atamaları var. Adım 5'te her class rename aynı zamanda JS'de de yapılmalı. | `ia-ptf.js`'de grep ile tüm eski class referansları bulunur, toplu replace yapılır. |
| **`epias-calculator.js` hardcode renkler** | JS'den `innerHTML` ile inject edilen `status-modern` span'larında inline renk veya class olabilir. | Adım 3'te `epias-calculator.js` taranır; bulunan inline style'lar badge class'larına çevrilir. |
| **`data-bs-theme="dark"` kaldırma anı** | Bu değişiklik sonrasında, `design-system.css` hazır olmadan bakılırsa sayfa karışık görünür. | Adım 0 ve Adım 1 aynı oturumda yapılır — araya kaydetme olmaz. |
| **Mobile breakpoints** | Light theme + card border'ları mobile'da beklenmedik şekilde davranabilir. | Adım 1 sonrasında 375px ve 768px'de kısa test. |

---

**Plan özeti:** 1 yeni dosya (`design-system.css`), 1 silinen dosya (`ia-ptf.css`), 3 sekmede inline style ve class temizliği, 2 JS dosyasında class rename. Sıra korunmalı — özellikle Adım 0 (dark theme removal) her şeyden önce.
